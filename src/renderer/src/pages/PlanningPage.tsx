...
export function PlanningPage(): React.JSX.Element {
  ...
  return (
    <div className={`planning-page ${section === 'projects' ? 'planning-page-projects' : ''}`}>
      ...
      {section === 'projects' && (areasQuery.isError || tagsQuery.isError) ? (
        <QueryFailure
          title="计划基础数据加载失败"
          error={areasQuery.error ?? tagsQuery.error}
          onRetry={() => void Promise.all([areasQuery.refetch(), tagsQuery.refetch()])}
        />
      ) : null}
      ...
      {section === 'practice' ? <PracticePage /> : <div className="planning-layout">
        <aside className="project-rail panel">
          ...
          {projectsQuery.isPending ? (
            ...
            <QueryFailure compact title="项目加载失败" error={projectsQuery.error} onRetry={() => void projectsQuery.refetch()} />
          ) : projects.length === 0 ? (
        ...
        <main key={selectedProjectId ?? 'no-project'} className="project-workspace panel">
          {!selectedProject ? (
            ...
            <>
              {tasksQuery.isError || milestonesQuery.isError || goalsQuery.isError ? (
                <QueryFailure
                  title="项目详情加载失败"
                  error={tasksQuery.error ?? milestonesQuery.error ?? goalsQuery.error}
                  onRetry={() => void Promise.all([tasksQuery.refetch(), milestonesQuery.refetch(), goalsQuery.refetch()])}
                />
              ) : null}
...
function TaskInspector({
  ...
  const quickUpdate = async (input: Parameters<typeof window.youtrace.planning.updateTask>[0]): Promise<void> => {
    await runInspectorAction(() => window.youtrace.planning.updateTask(input))
  }
  ...
  const queryFailed = checklistQuery.isError || dependenciesQuery.isError || recurrenceQuery.isError
  return <Dialog.Root open={task !== null} onOpenChange={onOpenChange}><Dialog.Portal><Dialog.Overlay className="dialog-overlay" /><Dialog.Content className="dialog-content task-inspector"><div className="dialog-heading"><div><span className="section-label">任务结构与快速操作</span><Dialog.Title>{task?.title}</Dialog.Title><Dialog.Description>子任务是独立对象；检查项是否参与进度由你明确选择。</Dialog.Description></div><Dialog.Close className="dialog-close" aria-label="关闭"><X size={18} /></Dialog.Close></div>{task && <div className="task-inspector-body">{queryFailed ? <QueryFailure compact title="任务详情加载失败" error={checklistQuery.error ?? dependenciesQuery.error ?? recurrenceQuery.error} onRetry={() => void Promise.all([checklistQuery.refetch(), dependenciesQuery.refetch(), recurrenceQuery.refetch()])} /> : null}<section><header><ListPlus size={15} /><strong>检查清单</strong><small>{checklistEnabled ? '检查项参与进度' : '默认不参与进度'}</small></header><label className="checklist-progress-mode"><input type="checkbox" checked={checklistEnabled} onChange={(event) => void runInspectorAction(() => window.youtrace.planning.setChecklistProgress({ taskId: task.id, enabled: event.target.checked }), () => setChecklistEnabled(event.target.checked))} />使用检查清单计算这个任务的进度</label><div className="checklist-items">{(checklistQuery.data ?? []).map((item) => <label key={item.id}><input type="checkbox" checked={item.checked} onChange={(event) => void runInspectorAction(() => window.youtrace.planning.updateChecklistItem({ id: item.id, checked: event.target.checked }))} /><input className="checklist-text-input" aria-label={`编辑检查项：${item.text}`} defaultValue={item.text} onBlur={(event) => { const text = event.target.value.trim(); if (!text || text === item.text) return; void runInspectorAction(() => window.youtrace.planning.updateChecklistItem({ id: item.id, text })) }} /><button type="button" aria-label={`删除检查项：${item.text}`} onClick={() => void runInspectorAction(() => window.youtrace.planning.deleteChecklistItem(item.id))}><X size={11} /></button></label>)}</div><div className="inline-create"><input aria-label="检查项内容" value={checkText} onChange={(event) => setCheckText(event.target.value)} /><button type="button" disabled={!checkText.trim()} onClick={() => void runInspectorAction(() => window.youtrace.planning.createChecklistItem({ taskId: task.id, text: checkText }), () => setCheckText(''))}><Plus size={12} />添加</button></div></section><section><header><Link2 size={15} /><strong>前置依赖</strong><small>未完成的前置项会显示阻塞来源</small></header><div className="dependency-list">{(dependenciesQuery.data ?? []).map((dependency) => <span key={dependency.prerequisiteTaskId}>{dependency.prerequisiteTitle}<em>{taskStatusLabels[dependency.prerequisiteStatus]}</em><button type="button" aria-label={`移除前置依赖：${dependency.prerequisiteTitle}`} onClick={() => void runInspectorAction(() => window.youtrace.planning.removeTaskDependency(task.id, dependency.prerequisiteTaskId))}><X size={11} /></button></span>)}</div><div className="form-row"><select aria-label="选择前置任务" value={dependencyId} onChange={(event) => setDependencyId(event.target.value)}><option value="">选择前置任务</option>{tasks.filter((candidate) => candidate.id !== task.id).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.title}</option>)}</select><input aria-label="覆盖原因" value={overrideReason} onChange={(event) => setOverrideReason(event.target.value)} placeholder="强制越过时的原因" /></div><button className="text-action" disabled={!dependencyId} onClick={() => void runInspectorAction(() => window.youtrace.planning.addTaskDependency({ taskId: task.id, prerequisiteTaskId: dependencyId, overrideReason: overrideReason || null }), () => { setDependencyId(''); setOverrideReason('') })}><Plus size={12} />添加依赖</button></section><section><header><Repeat2 size={15} /><strong>周期实例</strong><small>完成后生成独立的新实例，历史不变</small></header><div className="form-row form-row-three"><select aria-label="周期频率" value={frequency} onChange={(event) => setFrequency(event.target.value as typeof frequency)}><option value="daily">每日</option><option value="weekly">每周</option><option value="monthly">每月</option><option value="weekdays">工作日</option><option value="interval">自定义间隔</option></select><input aria-label="周期间隔" type="number" min="1" value={intervalValue} onChange={(event) => setIntervalValue(event.target.value)} /><input aria-label="下次发生日期" type="date" value={nextOccurrence} onChange={(event) => setNextOccurrence(event.target.value)} /></div><div className="inspector-actions"><button className="button button-secondary" onClick={() => void runInspectorAction(() => window.youtrace.planning.setTaskRecurrence({ taskId: task.id, rule: null }), () => setNextOccurrence(''))}>取消周期</button><button className="button button-secondary" disabled={!nextOccurrence} onClick={() => void runInspectorAction(() => window.youtrace.planning.setTaskRecurrence({ taskId: task.id, rule: { frequency, intervalValue: Number(intervalValue), weekdays: frequency === 'weekdays' ? [1, 2, 3, 4, 5] : [], nextOccurrence } }))}><Repeat2 size={12} />保存周期</button></div></section><section className="task-quick-actions"><button onClick={() => void quickUpdate({ id: task.id, status: 'blocked' })}>标为阻塞</button><button onClick={() => void quickUpdate({ id: task.id, startDate: addLocalDays(1) })}>转明日</button><button onClick={() => void quickUpdate({ id: task.id, dueAt: task.dueAt ? new Date(Date.parse(task.dueAt) + 86_400_000).toISOString() : new Date(Date.now() + 86_400_000).toISOString() })}>延期一天</button><button onClick={() => void quickUpdate({ id: task.id, includeInProgress: !task.includeInProgress })}>{task.includeInProgress ? '设为仅参考' : '纳入进度'}</button></section><div className="dialog-actions inspector-entity-actions"><button className="button button-secondary" type="button" onClick={() => onEdit(task)}><Pencil size={13} />编辑任务</button><button className="button button-danger" type="button" onClick={() => void onTrash(task)}><Trash2 size={13} />移至回收站</button></div>{error && <div className="inline-error">{error}</div>}</div>}</Dialog.Content></Dialog.Portal></Dialog.Root>
}
