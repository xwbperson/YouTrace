...
- 执行：plans、time_blocks、effort_entries、habit_rules、habit_instances、metric_entries。
- 证据：evidence、attachments、entity_attachments。
- 学习：course_profiles、textbooks、knowledge_items、mistakes、tests、reviews_queue。
...
数据库不保存附件绝对路径。路径规则、WAL 文件处理和备份流程见 [工作区与数据安全](05-工作区与数据安全.md)。
...
关闭主窗口时默认隐藏到系统托盘，主进程、计时和提醒调度继续运行。托盘仅提供必要入口；用户选择明确退出后才结束进程。应用再次启动或从休眠恢复时重新计算错过的提醒，并按规则汇总去重。提醒失败写入日志，但不能回滚任务或倒计时数据。托盘能力依据见 [Electron Tray](https://www.electronjs.org/docs/latest/api/tray/)。
